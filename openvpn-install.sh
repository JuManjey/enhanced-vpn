...
function installQuestions() {
	...
	# ==========================================================================
	# DPI / Restrictive network warning
	# ==========================================================================
	log_menu ""
	echo -e "${COLOR_YELLOW}╔══════════════════════════════════════════════════════════════╗${COLOR_RESET}"
	echo -e "${COLOR_YELLOW}║  WARNING: OpenVPN and DPI (Russia, China, Iran, etc.)       ║${COLOR_RESET}"
	echo -e "${COLOR_YELLOW}╠══════════════════════════════════════════════════════════════╣${COLOR_RESET}"
	echo -e "${COLOR_YELLOW}║  Standard OpenVPN has a distinctive TLS handshake that      ║${COLOR_RESET}"
	echo -e "${COLOR_YELLOW}║  DPI systems (TSPU in Russia) can identify and block.       ║${COLOR_RESET}"
	echo -e "${COLOR_YELLOW}║                                                              ║${COLOR_RESET}"
	echo -e "${COLOR_YELLOW}║  If clients will connect from Russia:                        ║${COLOR_RESET}"
	echo -e "${COLOR_YELLOW}║  → Use wireguard-install.sh with AmneziaWG instead.         ║${COLOR_RESET}"
	echo -e "${COLOR_YELLOW}║  → AmneziaVPN app (iOS/Android) + AmneziaWG config = works  ║${COLOR_RESET}"
	echo -e "${COLOR_YELLOW}║                                                              ║${COLOR_RESET}"
	echo -e "${COLOR_YELLOW}║  OpenVPN on TCP/443 with tls-crypt-v2 MAY work on some ISPs ║${COLOR_RESET}"
	echo -e "${COLOR_YELLOW}║  but is NOT guaranteed. For reliable access, use AmneziaWG.  ║${COLOR_RESET}"
	echo -e "${COLOR_YELLOW}╚══════════════════════════════════════════════════════════════╝${COLOR_RESET}"
	log_menu ""

	local DPI_CONTINUE=""
	read -rp "Continue with OpenVPN installation? [Y/n]: " -e -i y DPI_CONTINUE
	if [[ ${DPI_CONTINUE,,} == "n" ]]; then
		log_info "Exiting. Run ./wireguard-install.sh and select AmneziaWG for Russia."
		exit 0
	fi
	...
	# Detect IPv4 address — try external services first (handles NAT/cloud correctly)
	IP_IPV4=""
	if command -v curl &>/dev/null; then
		IP_IPV4=$(curl -4 -sS --connect-timeout 4 https://ifconfig.co 2>/dev/null) ||
			IP_IPV4=$(curl -4 -sS --connect-timeout 4 https://api.ipify.org 2>/dev/null) ||
			IP_IPV4=$(curl -4 -sS --connect-timeout 4 https://icanhazip.com 2>/dev/null) || true
	fi
	if [[ -z ${IP_IPV4} ]] || ! [[ ${IP_IPV4} =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
		IP_IPV4=$(ip -4 addr | sed -ne 's|^.* inet \([^/]*\)/.* scope global.*$|\1|p' | head -1)
	fi
	# Detect IPv6 address
	IP_IPV6=$(ip -6 addr | sed -ne 's|^.* inet6 \([^/]*\)/.* scope global.*$|\1|p' | head -1)
