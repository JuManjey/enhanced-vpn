...
function isUdpPortBusy() {
	...
	if command -v ss &>/dev/null; then
		if ss -Hlun "sport = :${PORT}" | grep -q .; then
			return 0
		fi
	fi
	return 1
...
function installQuestions() {
	...
	# UDP/443 is more censorship-resistant than random high ports in many networks
	RANDOM_PORT=443
	while true; do
...
function newClient() {
	...
	# If SERVER_ENDPOINT is IPv6, add brackets if missing
